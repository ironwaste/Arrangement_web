...
async function importPoomsaeExcel(db, workbook, eventId) {
  let success = 0;
  let failed = 0;
  let total = 0;
  const errors = [];

  const sheetTypeMap = { '个人品势': '个人', '混双品势': '混双', '团体品势': '团体' };

  for (const sheetName of workbook.SheetNames) {
    const poomsaeType = sheetTypeMap[sheetName];
    if (!poomsaeType) continue;
  ...
  return { success, failed, total, errors: errors.slice(0, 10) };
}
...
async function importKyougiExcel(db, workbook, eventId) {
  ...
  if (data.length < 2) return { success: 0, failed: 0, total: 0, errors: [] };
  ...
  const colMap = {};
  headers.forEach((h, i) => {
    if (h.includes('签号') || h.includes('序号')) colMap.signNo = i;
    if (h.includes('运动员号') || h.includes('编号')) colMap.athleteNo = i;
    if (h.includes('姓名') || h.includes('名字')) colMap.name = i;
    if (h.includes('性别')) colMap.gender = i;
    if (h.includes('单位')) colMap.unit = i;
    if (h.includes('组别')) colMap.group = i;
    if (h.includes('级别') || h.includes('体重')) colMap.weightClass = i;
  });
  ...
  return { success, failed, total: athletes.length, errors: errors.slice(0, 10) };
}
...
router.post('/athletes/import-excel', upload.single('file'), async (req, res) => {
  try {
    ...
    const athlete_type = req.body.athlete_type || 'taekwondo_kyougi';
    const workbook = xlsx.readFile(req.file.path);

    if (athlete_type === 'poomsae') {
      const result = await importPoomsaeExcel(db, workbook, event_id);
